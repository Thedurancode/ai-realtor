...
from app.services.zuckerbot_service import ZuckerbotService, get_zuckerbot_service
...
async def preview_campaign(request: CampaignPreviewRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def create_zuckerbot_campaign(request: CampaignCreateRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def launch_campaign(request: CampaignLaunchRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def pause_zuckerbot_campaign(campaign_id: str, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def get_campaign_performance(campaign_id: str, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def record_conversions(request: ConversionsRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def research_competitors(request: CompetitorResearchRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def research_market(request: MarketResearchRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def research_reviews(request: ReviewResearchRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def generate_creative(request: CreativeGenerateRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def get_creative_variants(request: CreativeVariantsRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def submit_creative_feedback(request: CreativeFeedbackRequest, service: ZuckerbotService = Depends(get_zuckerbot_service)):
...
async def get_zuckerbot_analytics_overview():
...
@router.get("/health")
async def zuckerbot_health_check(service: ZuckerbotService = Depends(get_zuckerbot_service)):
