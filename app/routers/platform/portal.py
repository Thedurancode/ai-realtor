...
async def get_my_properties(
    ...
    accesses = db.query(PropertyAccess).options(
        joinedload(PropertyAccess.property)
    ).filter(
...
async def portal_search_properties(
    query: str,
    current_user: PortalUser = Depends(get_current_user),
    db: Session = Depends(get_db)
):
...
async def get_property_by_identifier(
    ...
    # Batch-load access records for all matched properties to avoid N+1
    match_ids = [prop.id for prop in matches]
    accesses = db.query(PropertyAccess).filter(
        PropertyAccess.portal_user_id == current_user.id,
        PropertyAccess.property_id.in_(match_ids),
        PropertyAccess.is_active == True
    ).all()
    access_map = {a.property_id: a for a in accesses}
...
async def get_my_contracts(
    ...
    # Batch-load access records to avoid N+1 queries in the loop
    accesses = db.query(PropertyAccess).filter(
        PropertyAccess.portal_user_id == current_user.id,
        PropertyAccess.property_id.in_(accessible_property_ids)
    ).all()
    ...
    for contract in contracts:
        access = access_map.get(contract.property_id)
