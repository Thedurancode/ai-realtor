...
def create_voice_campaign(request: VoiceCampaignCreateRequest, db: Session = Depends(get_db)):
...
def list_voice_campaigns(status: str | None = None, limit: int = 100, db: Session = Depends(get_db)):
...
def pause_voice_campaign(campaign_id: int, db: Session = Depends(get_db)):
    campaign = _require_campaign(db, campaign_id)
